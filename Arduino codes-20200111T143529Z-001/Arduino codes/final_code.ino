long Time, Time1 = 0;
int f_bump = 29;
int m_bump = 31;
int b_bump = 33;

int value_fbump=0;
int value_mbump=0;
int value_bbump=0;

int harvester = 38;
int pwm_harvester = 5;

int seeder = 38;
int pwm_seeder = 5;

int sensor = 38;
int pwm_sensor = 5;

int main_state=0;

int x=23,y=35,z=37,r=25,d=15;

/////////////////////////GYRO////////////////////////
#include <SoftwareSerial.h>
SoftwareSerial mySerialrec (51, 53 );

double   n = 0;
int  q = 0;
int  k = 0;
char ch  ;
int  l = -1;
int  resetpin     = 14;
int  sig = 23;
int  a = 0;
int  stategyro    = 0;
double angle  = 0;
double  tempangle    = 0;
  
double correction = 0;
double angle_kp = 2.5, angle_kd = 3.9;

int p = 0;
int time0 = 0;

double t;
double angle_error = 0, angle_derror = 0, angle_perror = 0, angle_correction = 0;

void getangle()
{
  digitalWrite(sig, HIGH);
  while (!mySerialrec.available()) {
    Serial.println("sjhfdk");
  }
  //   while(mySerialrec.available()>0)
  //  {

  if ( mySerialrec.read() == 'a')
  {
    while (1)
    {
      ch = char(mySerialrec.read());
      if (ch == 'b')
      {
        break;
      }
      if (ch == '.')
      {
        q = 1;
      }
      if (ch == '-')
      {
        stategyro = 1;
      }
      int x = int(ch) - 48;
      if ((0 <= x && x < 10) )
      {
        if (q == 0) {
          n = n * 10 + x;
        }
        else
        {
          n += x * pow(10, l);
          l--;
        }

      }

      //     if(ch=='b')
      //     {break;}
    }

    digitalWrite(sig, LOW);
    if (stategyro == 1)
    {
      n = -n;
      stategyro = 0;
    }
    angle = n;
    n = 0;

    if (abs(tempangle - angle) > 30) {
      angle = tempangle;
    }
    tempangle = angle;
    Serial.println("angle");
    Serial.println(angle);

    q = 0;
    l = -1;
    if (a == 1)
    {
      angle -= 165;
    }
  }
}
/////////////////////////////ULTRA///////////////////////////////


#include <NewPing.h>
#define SONAR_NUM 8
#define MAX_DISTANCE 75
#define PING_INTERVAL 20

unsigned long pingTimer[SONAR_NUM]; // When each pings.
unsigned int ultradistance[SONAR_NUM]; // Store ping distances.
uint8_t currentSensor = 0; // Which sensor is active.

int frdistance = 0, fldistance = 0, topdistance = 0, brdistance = 0, front_height, middle_height, back_height ,  bldistance = 0;

double velocity_correction = 0, kp_velocity = 2.3, velocity_error = 0, kd_velocity = 3.5, velocity_derror = 0, velocity_perror = 0;
double dis_correction = 0, kp_dis = 2, dis_error = 0, kd_dis = 3.5, dis_derror = 0, dis_perror = 0;

NewPing sonar[SONAR_NUM] =
{ // Sensor object array.
  NewPing(A3, A2, MAX_DISTANCE),
  NewPing(A4, A5, MAX_DISTANCE),
  NewPing(A8, A9, MAX_DISTANCE),
  NewPing(A6, A7 , MAX_DISTANCE),
  NewPing(A0, A1, MAX_DISTANCE),
  NewPing(A14, A15, MAX_DISTANCE),
  NewPing(A12, A13 , MAX_DISTANCE),
  NewPing(44, 46, MAX_DISTANCE),

};
void pingdistance()
{

  for (uint8_t i = 0; i < SONAR_NUM; i++)
  {
    if (millis() >= pingTimer[i]) {
      pingTimer[i] += PING_INTERVAL * SONAR_NUM;
      if (i == 0 && currentSensor == SONAR_NUM - 1)
      {
        oneSensorCycle();
      } // Do something with results.
      sonar[currentSensor].timer_stop();
      currentSensor = i;
      ultradistance[currentSensor] = 0;
      sonar[currentSensor].ping_timer(echoCheck);
    }

  }

}
void echoCheck()
{ // If ping echo, set distance to array.
  if (sonar[currentSensor].check_timer())
    ultradistance[currentSensor] = sonar[currentSensor].ping_result / US_ROUNDTRIP_CM;

}

void oneSensorCycle()
{ // Do something with the results.
  fldistance = ultradistance[0];
  topdistance = ultradistance[1];
  frdistance = ultradistance[2];
  brdistance = ultradistance[3];
  bldistance = ultradistance[4];
  front_height = ultradistance[7];
  middle_height = ultradistance[6];
  back_height = ultradistance[5];

  for (uint8_t i = 0; i < SONAR_NUM; i++) {
    Serial.print(i);
    Serial.print("=");
    Serial.print(ultradistance[i]);
    Serial.println("cm ");
  }

}

///////////////////////WHEEL_MOTOR///////////////////////

double v1 = 0, v2 = 0, v3 = 0, v4 = 0;

int fr = 26;
int pwm1 = 4;
int fl = 43;
int pwm2 = 12;
int br = 32;
int pwm3 = 7;
int bl = 41;
int pwm4 = 11;


void backwheel_forward()
{
  digitalWrite(br, HIGH);
  analogWrite(pwm3, 60);
  digitalWrite(bl, HIGH);
  analogWrite(pwm4, 60);
}


///////////////////////////////////////////////////////////////////////

void backwheel_backward()
{
  digitalWrite(br, LOW);
  analogWrite(pwm3, 40);
  digitalWrite(bl, LOW);
  analogWrite(pwm4, 40);
}


///////////////////////////////////////

void frontwheel_forward()
{
  digitalWrite(fr, HIGH);
  analogWrite(pwm1, 60);
  digitalWrite(fl, HIGH);
  analogWrite(pwm2, 60);
}


////////////////////////////////////////

void frontwheel_backward()
{
  digitalWrite(fr, LOW);
  analogWrite(pwm1, 40);
  digitalWrite(fl, LOW);
  analogWrite(pwm2, 40);
}




////////////////////////////////LEADMOTORS///////////////////////////////////////

int lead_bm_error = 0, lead_bm_perror = 0, lead_bm_derror = 0, correction_lead_bm = 0;
double kp_lead_bm = 0.005, kd_lead_bm = 0;
int upstair_state = 0, variable = 0;

int lead_bf_error = 0, lead_bf_perror = 0, lead_bf_derror = 0, correction_lead_bf = 0;
double kp_lead_bf = 0.005, kd_lead_bf = 0;
int downstair_state = 0;

double m_pwm = 0, b_pwm = 0, f_pwm = 0;

int f = 40;
int pwmf = 8;
#define a1 21
#define b1  48
float theta1c = 0;
long int temp1, counter1 = 0;


int m = 38;
int pwmm = 5;
#define a2 18
#define b2  19
float theta2c = 0;
long int temp2, counter2 = 0;


int b = 49;
int pwmb = 6;
#define a3 2
#define b3  3
float theta3c = 0;
long int temp3, counter3 = 0;


void pid_bm_lead()
{
  lead_bm_error = counter3 - counter2;
  lead_bm_derror = lead_bm_error - lead_bm_perror;
  correction_lead_bm = kp_lead_bm * lead_bm_error + kd_lead_bm * lead_bm_derror;
  lead_bm_perror = lead_bm_derror;
  b_pwm = 111 - correction_lead_bm;
  m_pwm = 220 + correction_lead_bm;
}

void pid_bf_lead()
{
  lead_bf_error = counter3 - counter1;
  lead_bf_derror = lead_bf_error - lead_bf_perror;
  correction_lead_bf = kp_lead_bf * lead_bf_error + kd_lead_bf * lead_bf_derror;
  lead_bf_perror = lead_bf_derror;
  b_pwm = 111 + correction_lead_bf;
  f_pwm = 110 - correction_lead_bf;
}

void ai2() {
  if (digitalRead(b2) == !digitalRead(a2))
  {
    counter2--;
  } else {
    counter2++;
  }
}

////////////////////////////////////////////////////////////////////////////

void ai1() {
  if (digitalRead(b1) == !digitalRead(a1))
  {
    counter1--;
  } else {
    counter1++;
  }
}


/////////////////////////////////////////////////////////////////////////////


void ai3() {
  if (digitalRead(b3) == !digitalRead(a3))
  {
    counter3--;
  } else {
    counter3++;
  }
}

//////////////////////////////////////////////////////////////////////////////




void middle_lead_up(int b)
{
  digitalWrite(m, HIGH);
  analogWrite(pwmm, b);
}


/////////////////////////////////////////////////
void middle_lead_down(int a)
{
  digitalWrite(m, LOW);
  analogWrite(pwmm, a);
}

//////////////////////////////////////////////////
void front_lead_up(int c)
{
  digitalWrite(f, HIGH);
  analogWrite(pwmf, c);
}

/////////////////////////////////////////////////////////////////
void front_lead_down(int d)
{
  digitalWrite(f, LOW);
  analogWrite(pwmf, d);
}

///////////////////////////////////////////////////////////////////

void back_lead_up(int e)
{
  digitalWrite(b, HIGH);
  analogWrite(pwmb, e);
}

////////////////////////////////////////////////////////////////////////
void back_lead_down(int f)
{
  digitalWrite(b, LOW);
  analogWrite(pwmb, f);
}





////////////////////////////////////////////////////////////


void stopbot()
{
  analogWrite(pwm2, 0);
  analogWrite(pwm1, 0);
  analogWrite(pwm3, 0);
  analogWrite(pwm4, 0);
  analogWrite(pwmm, 0);
  analogWrite(pwmb, 0);
  analogWrite(pwmf, 0);
}

void setup()
{
  Serial.begin(115200); // start the serial port
  pingTimer[0] = millis() + 30; //first ping start in ms
  for (uint8_t i = 1; i < SONAR_NUM; i++)
    pingTimer[i] = pingTimer[i - 1] + PING_INTERVAL;
  while (millis() < 2000)
  {
    pingdistance();
  }

  mySerialrec.begin(115200);

  digitalWrite(resetpin, HIGH);
  delay(10);
  digitalWrite(resetpin, LOW);

  pinMode(fr, OUTPUT);
  pinMode(pwm1, OUTPUT);
  pinMode(fl, OUTPUT);
  pinMode(pwm2, OUTPUT);
  pinMode(br, OUTPUT);
  pinMode(pwm3, OUTPUT);
  pinMode(bl, OUTPUT);
  pinMode(pwm4, OUTPUT);
  pinMode(sig, OUTPUT);

  pinMode(f_bump, INPUT); //bump
  pinMode(m_bump, INPUT); //bump
  pinMode(b_bump, INPUT); //bump

  pinMode(a1, INPUT_PULLUP);
  pinMode(b1, INPUT_PULLUP);
  pinMode(f, OUTPUT);
  pinMode(pwmf, OUTPUT);
  pinMode(a2, INPUT_PULLUP);
  pinMode(b2, INPUT_PULLUP);
  pinMode(m, OUTPUT);
  pinMode(pwmm, OUTPUT);
  pinMode(a3, INPUT_PULLUP);
  pinMode(b3, INPUT_PULLUP);
  pinMode(b, OUTPUT);
  pinMode(pwmb, OUTPUT);
  attachInterrupt(digitalPinToInterrupt(a1), ai1, CHANGE);
  attachInterrupt(digitalPinToInterrupt(a2), ai2, CHANGE);
  attachInterrupt(digitalPinToInterrupt(a3), ai3, CHANGE);

  pinMode(x,INPUT_PULLUP);
  pinMode(y,INPUT_PULLUP);
  pinMode(z,INPUT_PULLUP);
  pinMode(r,INPUT_PULLUP);
  pinMode(d,INPUT_PULLUP);
}


int number_harvestor = 0;


void correctangle()
{
  getangle();
  angle_error = angle;
  angle_derror = angle_error  - angle_perror ;
  angle_perror = angle_error ;
  angle_correction = angle_kp * angle_perror + angle_kd * angle_derror;
  Serial.println("angle_correction");
  Serial.println(angle_correction);

}

void correctultra()
{
  if (fldistance > 0 && bldistance > 0 && number_harvestor == 0)
  {
    velocity_error = int((fldistance) - (bldistance));
    velocity_derror = velocity_error - velocity_perror;
    velocity_correction = kp_velocity * velocity_error + kd_velocity * velocity_derror;
    velocity_perror = velocity_error;

    dis_correction = 0;
    dis_error = 10 - (int(fldistance + bldistance) / 2);
    dis_derror = dis_error - dis_perror;
    dis_correction = kp_dis * dis_error + kd_dis * dis_derror;
    dis_perror = dis_error;
  }
  else if (frdistance > 0 && brdistance > 0 && number_harvestor == 1)
  {
    velocity_error = int(brdistance) - (frdistance);
    velocity_derror = velocity_error - velocity_perror;
    velocity_correction = kp_velocity * velocity_error + kd_velocity * velocity_derror;
    velocity_perror = velocity_error;

    dis_correction = 0;
    dis_error = -50 + (int(frdistance + brdistance) / 2);
    dis_derror = dis_error - dis_perror;
    dis_correction = kp_dis * dis_error + kd_dis * dis_derror;
    dis_perror = dis_error;
  }
  Serial.print("velerror=");
  Serial.println(velocity_error);
  Serial.print("disterror=");
  Serial.println(dis_error);
}

void straight()
{
  Serial.println("abcde3");
  //    if(angle!=0)
  //    {
  correctangle();
  //    }
  correctultra();
  correction = (angle_correction + velocity_correction) / 2;
  Serial.println("correction");
  Serial.println(correction);

  v1 = 60 - correction - dis_correction; //110
  v2 = 60 + correction + dis_correction;
  v3 = 60 - correction - dis_correction; //100
  v4 = 60 + correction + dis_correction; //100


  if (v3 > 255) {
    v3 = 255;
  }
  if (v4 > 255) {
    v4 = 255;
  }
  if (v1 > 255) {
    v1 = 255;
  }
  if (v2 > 255) {
    v2 = 255;
  }
  if (v3 < 0) {
    v3 = 0;
  }
  if (v4 < 0) {
    v4 = 0;
  }
  if (v1 < 0) {
    v1 = 0;
  }
  if (v2 < 0) {
    v2 = 0;
  }
  //    Serial.println("v1");
  //    Serial.println(v1);
  //    Serial.println("v2");
  //    Serial.println(v2);
  //    Serial.println("v3");
  //    Serial.println(v3);
  //    Serial.println("v4");
  //    Serial.println(v4);

  digitalWrite(fr, HIGH);
  analogWrite(pwm1, v1);
  digitalWrite(fl, HIGH);
  analogWrite(pwm2, v2);
  digitalWrite(br, HIGH);
  analogWrite(pwm3, v3);
  digitalWrite(bl, HIGH);
  analogWrite(pwm4, v4);
}

void rotateleft(int b)
{
  //   Serial.println("xyz");
  getangle();
  if (angle < 40)
  {
    digitalWrite(fr, HIGH);
    analogWrite(pwm1, 100);
    digitalWrite(br, HIGH);
    analogWrite(pwm3, 100);
    digitalWrite(fl, LOW);
    analogWrite(pwm2, 0);
    digitalWrite(bl, LOW);
    analogWrite(pwm4, 0);
  }
  else if (angle >= 40 && angle <= b)
  {
    digitalWrite(fr, HIGH);
    analogWrite(pwm1, 130);
    digitalWrite(br, HIGH);
    analogWrite(pwm3, 130);
    digitalWrite(fl, LOW);
    analogWrite(pwm2, 100);
    digitalWrite(bl, LOW);
    analogWrite(pwm4, 100);
  }

}

void rotateright(int a,int b)
{
  Serial.println("xyz");
  getangle();
  if (angle < a)
  {
    digitalWrite(fl, HIGH);
    analogWrite(pwm2, 100);
    digitalWrite(bl, HIGH);
    analogWrite(pwm4, 100);
    digitalWrite(fr, LOW);
    analogWrite(pwm1, 40);
    digitalWrite(br, LOW);
    analogWrite(pwm3, 40);
  }
  else if (angle >= a && angle <= b)
  {
    digitalWrite(bl, HIGH);
    analogWrite(pwm4, 130);
    digitalWrite(fl, HIGH);
    analogWrite(pwm2, 130);
    digitalWrite(fr, LOW);
    analogWrite(pwm1, 100);
    digitalWrite(br, LOW);
    analogWrite(pwm3, 100);
  }

}


void wallfollow_downstair()
{
  Serial.println("walldown");
  digitalWrite(harvester, HIGH);
  analogWrite(pwm_harvester, 200);

  if (fldistance > 0 && bldistance > 0 && number_harvestor == 0)
  {
    if (fldistance == bldistance )
    {
      angle = 0;
    }
    Serial.println("abcde");
    if ( topdistance == 0 || topdistance > 60 )
    {
      Serial.println("abcde2");
      straight();
    }
    else if (topdistance < 60 && topdistance != 0)
    {
      while (angle < 165)
      {
        Serial.println("fghij");
        rotateright(40,165);
      }

      angle = 0;
      int time5 = millis();
      while (millis() - time5 < 2000)
      {
        stopbot();
        a = 1;
        getangle();
        pingdistance();
        Serial.println("klmno");
      }
      number_harvestor = 1;
    }
  }

  else if (frdistance > 0 && brdistance > 0 && number_harvestor == 1)
  {
    if (frdistance == brdistance )
    {
      angle = 0;
    }
    Serial.println("pqrst");
    if (topdistance > 40 || topdistance == 0)
    {
      straight();
    }
    else if (topdistance < 40)
    {
      while (angle < 252)
      {
        pingdistance();
        rotateright(200,255);
      }
     
    }
    else
    {
      int time5 = millis();
      while (millis() - time5 < 2000)
      {
        stopbot();
        pingdistance();
        Serial.println("klmno");
      } number_harvestor = 2;
    }
  }



  else if (number_harvestor == 2)
  {
    stopbot();
    main_state=2;
  }
}

void downstair()
{
   Serial.println("downstair");

  if ( downstair_state == 0)
  {
    if (front_height < 20)
    {
      
      backwheel_forward();
      frontwheel_forward();
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      downstair_state = 1;
    }
  

  }
  else if ( downstair_state == 1 )
  {
          if (counter1 < 43000 && f_bump == 0)
      {
        front_lead_down(200);
      }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
      pingdistance();
      }
      downstair_state = 2;
    }
  }

  
  else if (downstair_state == 2)
  {
    if (middle_height < 40)
    {
      backwheel_forward();
      frontwheel_forward();
    }

    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      downstair_state = 3;
    }
  }

  else  if (downstair_state == 3)
  {
    if (counter2 < 44000 && m_bump == 0)
    {
       middle_lead_down(180);
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
      pingdistance();
      }
      downstair_state = 4;
    }
  }

else if(downstair_state==4)
{
      Time1=millis();
      while(millis()-Time1<300 )
      {
       front_lead_down(200);
      }
      analogWrite(pwm1,0);
      analogWrite(pwm2,0);
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
    downstair_state = 5;
    }
  else if (downstair_state == 5)
  {
    if (back_height < 25)
    {
      backwheel_forward();
      frontwheel_forward();
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      downstair_state = 6;
    }
   
  }

else if (downstair_state == 6){
  
    if (counter3 < 42000 && b_bump == 0)
    {
       back_lead_down(200);
    }

        else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 1000) {
        pingdistance();
      }
      downstair_state = 7;
    }
  }
  
else if (downstair_state==7){
    if ( counter2 > 1000 && m_bump == 0)
    {
     middle_lead_up(150);
    }
 
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 1000) {
        pingdistance();
      }
      downstair_state = 8;
    }
  }

 else if ( downstair_state == 8)
  {
    if (middle_height >19)
    {
    pid_bf_lead();
      if (counter1 > 600 && f_bump == 0)
      {
        front_lead_up(f_pwm);
      }
      else
      {
        front_lead_up(0);
      }

      if (counter3 > 600 && b_bump == 0)
      {
        back_lead_up(b_pwm);
      }
      else
      {
        back_lead_up(0);
      }
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      downstair_state = 9;
    }
  }
  else if(downstair_state == 9){
    while(angle<90){
    pingdistance();
    rotateleft(90);
    }
  }

  else
  {
    stopbot();
    main_state=1;
  }
  //
  //
//            Serial.print("1==");
//            Serial.println(counter1);
  //          Serial.print("2==");
  //          Serial.println(counter2);
  //          Serial.print("3==");
  //          Serial.println(counter3);
}



int number_seeder = 0;

void wallfollow_seeder()
{
 Serial.println("seeder");

  if (fldistance > 0 && bldistance > 0 && number_seeder == 0)
  {
    if (fldistance == bldistance )
    {
      angle = 0;
    }
    Serial.println("abcde");
    if ( topdistance == 0 || topdistance > 60 )
    {
      Serial.println("abcde2");
      straight();
      digitalWrite(seeder, HIGH);
      analogWrite(pwm_seeder, 200);
    }
    else if (topdistance < 60 && topdistance != 0)
    {


      while (front_height < 25)
      {
        pid_bf_lead();
        if (counter1 < 60000 && f_bump == 0)
        {
          //            Serial.println(f_pwm);
          front_lead_down(f_pwm);
        }
        else
        {
          front_lead_down(0);
        }

        if (counter3 < 60000 && b_bump == 0)
        {
          //            Serial.println("ijndj");
          back_lead_down(b_pwm);
        }
        else
        {
          back_lead_down(0);
        }
      }
      //      Serial.println("fghij");

      while (angle < 165)
      {
        rotateright(40, 165);
      }


      while (middle_height > 19)
      {
        pingdistance();
        pid_bf_lead();
        if (counter1 > 600 && f_bump == 0)
        {
          front_lead_up(f_pwm);
        }
        else
        {
          front_lead_up(0);
        }

        if (counter3 > 600 && b_bump == 0)
        {
          back_lead_up(b_pwm);
        }
        else
        {
          back_lead_up(0);
        }
      }

    }
    else
    {
      stopbot();
      number_seeder = 1;
    }
  }

  else if (frdistance > 0 && brdistance > 0 && number_seeder == 1)
  {
    if (frdistance == brdistance )
    {
      angle = 0;
    }
    if (topdistance > 40 || topdistance == 0)
    {
      straight();
    }
    else if (topdistance < 40)
    {
      while (angle < 330)
      {
        rotateright(200, 335);
      }
    }
  }



  else
  {
    stopbot();
    main_state=4;
    number_seeder = 2;
  }
}


int number_sensor=0;

 void wallfollow_sensor() 
 {
  Serial.println("sensor");
    digitalWrite(sensor,HIGH);
    analogWrite(pwm_sensor,200);
    
    if(fldistance>0 && bldistance>0 && number_sensor==0)
    {
        if(fldistance == bldistance )
         {
            angle=0;
         }
        Serial.println("abcde");
        if( topdistance==0 || topdistance>60 )
        {
           Serial.println("abcde2");
          straight();
        }
        else if(topdistance<60 && topdistance!=0)
        {
          while(angle<165)
          {
            Serial.println("fghij");
            pingdistance();
            rotateright(40,165);
          }
           number_sensor=1;
        }
    }
    
    else if(frdistance>0 && brdistance>0 && number_sensor==1)
    {
        if(frdistance == brdistance )
         {
            angle=0;
         }
        if(topdistance>40 ||topdistance==0)
        {
          straight();
        }
          number_sensor=2;
        }
    



    else if(number_sensor==2)
    {
      stopbot();
      main_state=5;
    }
 }

 void upstair()
{
  Serial.println("upstair");
  if ( upstair_state == 0)
  {
    if (front_height < 50)
    {
      pid_bm_lead();
      if (counter2 < 60000 && m_bump == 0)
      {
        Serial.println(m_pwm);
        middle_lead_down(m_pwm);
      }
      else
      {
        middle_lead_down(0);
      }

      if (counter3 < 60000 && b_bump == 0)
      {
        Serial.println("ijndj");
        back_lead_down(b_pwm);
      }
      else
      {
        back_lead_down(0);
      }
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      upstair_state = 1;
    }
  }


  else if ( upstair_state == 1 )
  {

    if (front_height > 25 )
    {
      Serial.println("gbvhybg");
      analogWrite(f_pwm,0);
      analogWrite(m_pwm,0);
      analogWrite(b_pwm,0);
      backwheel_forward();
    }
    else
    {

      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      upstair_state = 2;
    }
  }
  else if (upstair_state == 2)
  {
    if ( counter2 > 0 && m_bump == 0)
    {
     
      middle_lead_up(150);
    }

    else
    {
            Serial.println("234567");
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      upstair_state = 3;
    }
  }
  //

  else  if (upstair_state == 3)
  {
    if (middle_height > 25)
    {
      analogWrite(f_pwm,0);
      analogWrite(m_pwm,0);
      analogWrite(b_pwm,0);
      backwheel_forward();
      frontwheel_forward();
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
      upstair_state = 4;
    }
  }


  else if (upstair_state == 4)
  {
    if (counter3 >600 && b_bump == 0)
    {
      back_lead_up(150);
    }
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
        Serial.println("abcdefgh");
      }
      upstair_state = 5;
//      delay(2000);
    }
   
  }

  else if (upstair_state == 5){
  
      Time1=millis();
      while(millis()-Time1<200 )
      {
        
       digitalWrite(f,LOW);
      analogWrite(pwmf,100);
      }
      analogWrite(pwmf,0);
      stopbot();
      Time = millis();
      while (millis() - Time < 2000) {
        pingdistance();
      }
     upstair_state=6;
  }
  
else if (upstair_state==6){
    if (back_height > 25)
    {
//      Serial.println("dhruv");
      frontwheel_forward();
      backwheel_forward();
    }
 
    else
    {
      stopbot();
      Time = millis();
      while (millis() - Time < 1000) {
        pingdistance();
      }
      upstair_state = 7;
    }
  }

  else if (upstair_state==7){

      Time = millis();
      while (millis() - Time < 1000) {
      frontwheel_forward();
      backwheel_forward();
      }
      upstair_state = 8;
    }
 

  else if (upstair_state==8){
      while (angle<90) {
        pingdistance();
        rotateright(40,90);
      }
      upstair_state = 9;
    }
  else
  {
    stopbot();
    main_state=3;
  }
  //
  //
  //      //    Serial.print("1==");
  //      //    Serial.println(counter1);
  //          Serial.print("2==");
  //          Serial.println(counter2);
  //          Serial.print("3==");
  //          Serial.println(counter3);

}

void loop()
{
  value_fbump=digitalRead(f_bump);
  value_bbump=digitalRead(b_bump);
  value_mbump=digitalRead(m_bump);
  pingdistance();

 if(digitalRead(d)==LOW )
  {
    Serial.println("downstair");
     downstair();
  }

   else if(digitalRead(y)==LOW)
  {
     Serial.println("upstair");
    upstair();
  }
    else if( digitalRead(z)==LOW)
  {
     Serial.println("wallfollowseeder");
    wallfollow_seeder();
  }
      else if(digitalRead(x)==LOW)
  {
    Serial.println("wallfollowsensor");
    wallfollow_sensor();
  }

    else if( digitalRead(r)==LOW)
  {
    Serial.println("wallfollow_downstair");
    wallfollow_downstair();
  }

  else
  {
      Serial.println("stopbot");
    stopbot();
  }
//  wallfollow();
//  upstair();
}
 
